...
// change to hashmap to store address
impl Names {
    fn new() -> Self {
...
async fn accept_loop(listener: TcpListener, tx: Sender<(String)>, names: Names, rooms: Rooms) {
    loop {
        ...
        tokio::select! {
            ...
            user_msg = reader.read_line(&mut line) => {
                if user_msg.unwrap() == 0 {
                    ...
                    if (name_changed) {
                        names.remove(&name);
                        let _ = room_tx.send(format!("📢: ({name}) has changed their name to ({new_name})\n")).unwrap();
                        name = new_name;
                    } else {
